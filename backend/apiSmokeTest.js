 ...
 *
 * FEATURES:
 * - Automatically fetches Google credentials for adar@bahar.co.il and adarb@bahar.co.il
 * - Caches tokens locally to avoid repeated authentication
 * - Supports manual credential override via environment variables
 * - Comprehensive test coverage for all API endpoints
 * - Automatic cleanup of test artifacts (bull pens)
 *
 * Usage examples:
 *   # Auto-fetch tokens (requires ADAR_PASSWORD and ADARB_PASSWORD env vars)
 ...
 *
 *   # Interactive mode (prompts for tokens if auto-fetch fails)
 *   node backend/apiSmokeTest.js --interactive --base-url=http://localhost:4000
 *
 *   # List all available tests
 *   node backend/apiSmokeTest.js --list
 *
 * ENVIRONMENT VARIABLES:
 *   - ADAR_PASSWORD: Password for adar@bahar.co.il (required for auto-fetch)
 *   - ADARB_PASSWORD: Password for adarb@bahar.co.il (required for auto-fetch)
 *   - TEST_GOOGLE_CREDENTIAL: Override primary credential (optional)
 *   - TEST_GOOGLE_CREDENTIAL_2: Override secondary credential (optional)
 *   - API_BASE_URL: Default base URL (default: http://localhost:4000)
 *
...
addTest(
  'market-data-search-symbols',
  'GET /api/market-data/search?q=AAPL',
  'Search for stock symbols by query string',
  async (ctx) => {
    const res = await httpRequest('GET', `${ctx.baseUrl}/api/market-data/search?q=AAPL`);
    if (res.status !== 200) throw new Error(`Expected 200, got ${res.status}`);
    const body = res.json || {};
    if (!Array.isArray(body.result)) {
      throw new Error('Expected result array in search response');
    }
    if (typeof body.count !== 'number') {
      throw new Error('Expected count in search response');
    }
    if (body.result.length === 0) {
      throw new Error('Expected at least one result for AAPL search');
    }
    const firstResult = body.result[0];
    if (!firstResult.symbol || !firstResult.description) {
      throw new Error('Search result missing symbol or description');
    }
  }
);
...
addTest(
  ...
  async (ctx) => {
    if (!ctx.googleCredential) {
      throw new Error('No Google credential provided for budget-get-current test');
    }
    ...
    const res = await httpRequest('GET', `${ctx.baseUrl}/api/v1/budget`, {
      headers: { Authorization: `Bearer ${authBody.token}` },
    });
    ...
    if (typeof body.balance !== 'number') {
      throw new Error('Budget response missing balance');
    }
    if (!body.currency) {
      throw new Error('Budget response missing currency');
    }
  },
  { optional: true }
);
...
addTest(
  ...
  async (ctx) => {
    if (!ctx.googleCredential) {
      throw new Error('No Google credential provided for budget-get-logs test');
    }
    ...
    const res = await httpRequest('GET', `${ctx.baseUrl}/api/v1/budget/logs?limit=10&offset=0`, {
      headers: { Authorization: `Bearer ${authBody.token}` },
    });
    ...
    if (!Array.isArray(body.logs)) {
      throw new Error('Budget logs response missing logs array');
    }
    if (typeof body.total !== 'number') {
      throw new Error('Budget logs response missing total count');
    }
  },
  { optional: true }
);
...
// ============================================================================
// BONUS/PROMOTION TESTS
// ============================================================================

addTest(
  'bonus-get-my-bonuses',
  'GET /api/v1/bonus/my-bonuses',
  'Get user\'s redeemed bonuses',
  async (ctx) => {
    if (!ctx.googleCredential) {
      throw new Error('No Google credential provided for bonus-get-my-bonuses test');
    }
    ...
    const res = await httpRequest('GET', `${ctx.baseUrl}/api/v1/bonus/my-bonuses`, {
      headers: { Authorization: `Bearer ${authBody.token}` },
    });
    ...
    if (!Array.isArray(body.bonuses)) {
      throw new Error('My bonuses response missing bonuses array');
    }
  },
  { optional: true }
);
...
addTest(
  'bonus-redeem-invalid-code',
  'POST /api/v1/bonus/redeem',
  'Attempt to redeem invalid promotion code, expect 400 or 404',
  async (ctx) => {
    if (!ctx.googleCredential) {
      throw new Error('No Google credential provided for bonus-redeem-invalid-code test');
    }
    ...
    const res = await httpRequest('POST', `${ctx.baseUrl}/api/v1/bonus/redeem`, {
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${authBody.token}` },
      body: JSON.stringify({ code: 'INVALID_CODE_XYZ_123' }),
    });
...
addTest(
  ...
  'Get leaderboard for a bull pen',
  async (ctx) => {
    if (!ctx.googleCredential) {
      throw new Error('No Google credential provided for leaderboard-get test');
    }
    ...
    // Create a bull pen first
    const createRes = await httpRequest('POST', `${ctx.baseUrl}/api/bull-pens`, {
      headers: { 'Content-Type': 'application/json', ...authHeader },
      body: JSON.stringify({
        name: 'SmokeTest_Leaderboard',
        description: 'Smoke test - leaderboard',
        durationSec: 3600,
        maxPlayers: 10,
        startingCash: 10000,
        allowFractional: true,
    ...
    // Get leaderboard
    const res = await httpRequest('GET', `${ctx.baseUrl}/api/bull-pens/${bullPenId}/leaderboard`, {
      headers: authHeader,
    });
    ...
    if (!Array.isArray(body.leaderboard)) {
      throw new Error('Leaderboard response missing leaderboard array');
    }
  },
  { optional: true }
);
...
addTest(
  'holdings-authorized-crud',
  'CRUD /api/holdings (authorized)',
  'Create, read, update, and delete a holding',
  async (ctx) => {
    if (!ctx.googleCredential) {
      throw new Error('No Google credential provided for holdings-authorized-crud test');
    }
    ...
    // Create a holding
    const createRes = await httpRequest('POST', `${ctx.baseUrl}/api/holdings`, {
      headers: { 'Content-Type': 'application/json', ...authHeader },
      body: JSON.stringify({
        ticker: 'SmokeTest_HOLD',
        name: 'Smoke Test Holding',
        shares: 100,
        purchasePrice: 50.25,
    ...
    if (createRes.status !== 201) {
      throw new Error(`Expected 201 from create holding, got ${createRes.status}`);
    }
    const created = (createRes.json || {}).holding;
    if (!created || !created.id) {
      throw new Error('Create holding response missing holding.id');
    }

    const holdingId = created.id;

    // Read the holding
    const getRes = await httpRequest('GET', `${ctx.baseUrl}/api/holdings`, {
      headers: authHeader,
    });

    if (getRes.status !== 200) {
      throw new Error(`Expected 200 from get holdings, got ${getRes.status}`);
    }
    const getBody = getRes.json || {};
    if (!Array.isArray(getBody.holdings)) {
      throw new Error('Get holdings response missing holdings array');
    }
    const found = getBody.holdings.find((h) => h.id === holdingId);
    if (!found) {
      throw new Error('Created holding not found in holdings list');
    }

    // Update the holding
    const updateRes = await httpRequest('PUT', `${ctx.baseUrl}/api/holdings/${holdingId}`, {
      headers: { 'Content-Type': 'application/json', ...authHeader },
      body: JSON.stringify({
        ticker: 'SmokeTest_HOLD',
        name: 'Smoke Test Holding Updated',
        shares: 150,
        purchasePrice: 51.25,
    ...
    if (updateRes.status !== 200) {
      throw new Error(`Expected 200 from update holding, got ${updateRes.status}`);
    }
    const updated = (updateRes.json || {}).holding;
    if (!updated || updated.shares !== 150) {
      throw new Error('Updated holding does not have correct shares');
    }

    // Delete the holding
    const deleteRes = await httpRequest('DELETE', `${ctx.baseUrl}/api/holdings/${holdingId}`, {
      headers: authHeader,
    });

    if (deleteRes.status !== 204) {
      throw new Error(`Expected 204 from delete holding, got ${deleteRes.status}`);
    }
  },
  { optional: true }
);
...
addTest(
  ...
  'GET /api/admin/users',
  'List users without admin privileges should return 403',
  async (ctx) => {
    if (!ctx.googleCredential) {
      throw new Error('No Google credential provided for admin-list-users-unauthorized test');
    }
    ...
    const res = await httpRequest('GET', `${ctx.baseUrl}/api/admin/users`, {
      headers: { Authorization: `Bearer ${authBody.token}` },
    });

    // Non-admin users should get 403
    if (res.status !== 403) {
      throw new Error(`Expected 403 from non-admin user, got ${res.status}`);
    }
  },
  { optional: true }
);
...
addTest(
  'admin-rake-config-unauthorized',
  'GET /api/admin/rake/config',
  'Get rake config without admin privileges should return 403',
  async (ctx) => {
    if (!ctx.googleCredential) {
      throw new Error('No Google credential provided for admin-rake-config-unauthorized test');
    }
    ...
    const res = await httpRequest('GET', `${ctx.baseUrl}/api/admin/rake/config`, {
      headers: { Authorization: `Bearer ${authBody.token}` },
    });
...
addTest(
  'admin-achievement-rules-unauthorized',
  'GET /api/admin/achievement-rules',
  'List achievement rules without admin privileges should return 403',
  async (ctx) => {
    if (!ctx.googleCredential) {
      throw new Error('No Google credential provided for admin-achievement-rules-unauthorized test');
    }
    ...
    const res = await httpRequest('GET', `${ctx.baseUrl}/api/admin/achievement-rules`, {
      headers: { Authorization: `Bearer ${authBody.token}` },
    });
...
async function main() {
  ...
  // Try to auto-fetch tokens for specific email addresses if not provided
  if (!googleCredential || isTokenExpired(googleCredential)) {
    console.log(`${colors.cyan}Attempting to fetch token for adar@bahar.co.il...${colors.reset}`);
    try {
      const freshToken = await fetchTokenForEmail('adar@bahar.co.il', process.env.ADAR_PASSWORD || '');
      if (freshToken) {
        googleCredential = freshToken;
        console.log(`${colors.green}✓${colors.reset} Fetched TEST_GOOGLE_CREDENTIAL for adar@bahar.co.il`);
      }
    } catch (err) {
      console.log(`${colors.yellow}⚠️  Could not auto-fetch token for adar@bahar.co.il${colors.reset}`);
    }
  }

  if (!secondGoogleCredential || isTokenExpired(secondGoogleCredential)) {
    console.log(`${colors.cyan}Attempting to fetch token for adarb@bahar.co.il...${colors.reset}`);
    try {
      const freshToken = await fetchTokenForEmail('adarb@bahar.co.il', process.env.ADARB_PASSWORD || '');
      if (freshToken) {
        secondGoogleCredential = freshToken;
        console.log(`${colors.green}✓${colors.reset} Fetched TEST_GOOGLE_CREDENTIAL_2 for adarb@bahar.co.il`);
      }
    } catch (err) {
      console.log(`${colors.yellow}⚠️  Could not auto-fetch token for adarb@bahar.co.il${colors.reset}`);
    }
  }
  ...
  const googleCredentialExpired = googleCredential && checkTokenExpiry(googleCredential).expired;
  const secondGoogleCredentialExpired = secondGoogleCredential && checkTokenExpiry(secondGoogleCredential).expired;

  for (const t of tests) {
    if (t.optional && (!googleCredential || googleCredentialExpired)) {
      console.log(`${colors.yellow}SKIP${colors.reset} ${t.id} (${t.endpoint}) - no valid Google credential provided`);
      skipCount += 1;
      continue;
    }
